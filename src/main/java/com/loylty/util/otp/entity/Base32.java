...
public class Base32
	{
		...
		private static final Base32			INSTANCE	= new Base32("ABCDEFGHIJKLMNOPQRSTUVWXYZ234567");	// RFC
																											// 4648/3548
		...
		protected byte[] decodeInternal(String encoded) throws DecodingException
			{
				...
				// Remove padding. Note: the padding is used as hint to
				// determine how many
				// bits to decode from the last incomplete chunk (which is
				// commented out
		...
		protected String encodeInternal(byte[] data)
			{
				...
				// SHIFT is the number of bits per output character, so the
				// length of the
				// output is the length of the input multiplied by 8/SHIFT,
				// rounded up.
				if (data.length >= (1 << 28))
					{
